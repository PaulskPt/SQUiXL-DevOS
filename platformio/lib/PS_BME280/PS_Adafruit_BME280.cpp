...
bool Adafruit_BME280::begin(uint8_t addr, TwoWire *theWire) {
  if (spi_dev == NULL) {
    ...
    if (i2c_dev)
      delete i2c_dev;
    // addition by @PaulskPt
    if (addr == 0) {
      // default address
      addr = BME280_ADDRESS;
    } else if (addr != BME280_ADDRESS &&
             addr != BME280_ADDRESS_ALTERNATE) {
      // invalid address
      return false;
    }
    _i2caddr = addr; // store the address for later use
    // end addition by @PaulskPt
...
// Addition by @PaulskPt
void Adafruit_BME280::end() {
  if (spi_dev) {
    ; // No end() method for Adafruit_SPIDevice, nothing to do here
  } else if (i2c_dev) {
    // if we have an I2C device, we can end it
    if (i2c_dev->detected()) {
      i2c_dev->end();
    }
  }
...
// Addition by @PaulskPt
uint8_t Adafruit_BME280::I2Caddress(void) {
